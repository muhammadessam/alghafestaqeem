@unless($tags->isEmpty())
<div class="my-4">
    <h4 class="mb-4 text-xl font-bold text-gray-700 dark:text-gray-200">{{ __('Categories') }}</h4>
        ...
        <ul>
            ...
            <li class="px-1 py-4 border-b border-t border-white hover:border-primary-600 transition duration-300">
                <a href="{{ route('tags',['category',$tag->slug]) }}" class="flex items-center text-gray-600 cursor-pointer">
                    {{ $tag->name }}
                    <span class="text-gray-500 ltr:ml-auto rtl:mr-auto">{{ $tag->posts_published_count }} <span class="text-xs">{{ __('Post') }}</span></span>
