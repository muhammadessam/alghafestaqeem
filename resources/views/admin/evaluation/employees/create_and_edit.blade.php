...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="card-name">@lang('admin.EvaluationEmployees') /
                    @if ($item->id)
                ...
                <hr>
                @if ($item->id)
                <form class="needs-validation" action="{{ route('admin.evaluation-employees.update', $item->id) }}" method="POST" enctype='multipart/form-data' novalidate>
                    <input type="hidden" name="_method" value="PUT">
                    @else
                    <form class="needs-validation" action="{{ route('admin.evaluation-employees.store') }}" method="POST" enctype='multipart/form-data' novalidate>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="col-md-12">
                                    <div class="form-group ">
                                        <label for="price-field"> @lang('admin.price')</label>
                                        <input id="price-field" min="0" step="any" type="number" class="form-control @if ($errors->has('price')) is-invalid @endif" name='price' value="{{ $item->price ?? old('price') }}" />
                                        @if ($errors->has('price'))
                                        <span class="invalid-feedback">{{ $errors->first('price') }}</span>
                                        @else
                                        <div class="invalid-feedback">please add price
                                        </div>
                                        @endif
                                    </div>
                                </div>
    ...
    <div class="col-md-12">
        ...
        <a class="btn btn-danger pull-right text-white" href="{{ route('admin.evaluation-employees.index') }}">@lang('admin.Cancel') <i class="icon-arrow-left-bold"></i></a>
    </div>
</div>
</form>
</div>
