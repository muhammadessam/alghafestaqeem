...
<div class="row">
    <div class="col-lg-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">@lang('admin.EvaluationEmployees') <span class="badge badge-danger">{{ $result['counts'] ?? '0' }}</span>
                    @if (can('evaluation-employees.create'))
                    <a class="btn btn-success pull-right text-white" href="{{ route('admin.evaluation-employees.create') }}">
                        <i class=" icon-plus"></i>
                    </a>
                    @endif
                </h4>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-left">
                                    ...
                                    <a class="btn btn-sm btn-warning" href="{{ route('admin.evaluation-employees.edit', $item->id) }}">
                                        <i class="text-white icon-note"></i>
                                    </a>
                                    @endif
                                    @if (can('evaluation-employees.destroy'))
                                    <form action="{{ route('admin.evaluation-employees.destroy', $item->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Delete? Are you sure?');">
                                        {{ csrf_field() }}
